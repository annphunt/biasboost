...
def _create_attempt(client, db, user_id: int, bias: str = "Confirmation Bias"):
...
def test_save_answer_valid(client, db):
    attempt_id = _create_attempt(client, db, 400)
...
def test_save_answer_all_answered_marks_complete(client, db):
    attempt_id = _create_attempt(client, db, 401)
    for i in range(1, 5):
...
def test_save_answer_not_all_answered(client, db):
    attempt_id = _create_attempt(client, db, 402)
...
def test_save_answer_invalid_letter(client, db):
    attempt_id = _create_attempt(client, db, 403)
...
def test_save_answer_question_not_found(client, db):
    attempt_id = _create_attempt(client, db, 404)
